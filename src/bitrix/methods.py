...
async def get_checklist_summary(bx: BitrixClient, task_id: int) -> ChecklistSummary:
    ...
    Допущение: иерархия ровно двухуровневая (этап -> его прямые пункты), как на живом
    портале (проверено на checklist.json). Внуки (уровень >2, т.е. дети не-корневых
    пунктов) в stage_done/stage_total НЕ учитываются — они на портале не встречаются;
    появятся — потребуется рекурсивный подсчёт вместо плоского children_by_root."""
