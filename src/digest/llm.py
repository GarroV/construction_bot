...
_LOCALE_SCHEMA = {
    ...
    "properties": {
        "language": {
            ...
            "enum": ["ru", "en"],
            "description": (
                "'ru', если ТЕКСТ комментариев обсуждения преимущественно на русском; "
                "иначе 'en' — в том числе когда комментариев нет вовсе. Название карточки "
                "и имена авторов для этого поля значения не имеют (игнорируй их язык)."
            ),
        },
        "timezone": {
            "type": "string",
            "description": (
                ...
                "пустая строка, если город неясен. Единственный источник для этого поля — "
                "название карточки, обсуждение не используется."
            ),
        },
    },
...
# Формулировка ниже уточнена по живому кейсу Черногории (фидбек владельца): карточка
# "Podgorica-2" и авторы комментариев ("Cheredanova Kristina", "Юрьев Вадим") латиницей,
# но вся коммуникация по существу на русском — язык обязан определяться СТРОГО по тексту
# комментариев, а не по названию/именам (те бывают латиницей и у русскоязычной команды).
# Название карточки при этом остаётся ЕДИНСТВЕННЫМ источником для таймзоны — две задачи
# полей нарочно разведены по разным кускам входа, чтобы модели было сложнее их смешать.
# Пустое обсуждение не даёт никакого сигнала о языке команды — 'en' как безопасный дефолт
# (владелец: «любой другой язык (включая отсутствие сигнала) -> english»), симметрично
# основному правилу "иначе 'en'" и не требует отдельной ветки в _parse_locale_response.
_LOCALE_PROMPT_TEMPLATE = """Определи для чата стройки два значения:

1. language — язык КОМАНДЫ, определяй СТРОГО по тексту комментариев обсуждения ниже (что
реально написано по существу), а не по названию карточки и не по именам авторов — оба
нередко латиницей даже у полностью русскоязычной команды (пример из практики: карточка
называется «Podgorica-2», авторы комментариев подписаны как «Cheredanova Kristina» и
«Юрьев Вадим», но пишут по-русски — правильный ответ 'ru'). Игнорируй язык названия
карточки и язык имён/фамилий авторов целиком, они не являются сигналом.
   - 'ru' — если текст комментариев преимущественно на русском.
   - 'en' — если преимущественно на любом другом языке, ИЛИ если комментариев нет
     (пусто/"-"): без обсуждения нет сигнала о языке команды, безопасный дефолт — 'en'.
   Третьих значений нет.

2. timezone — IANA-таймзона города, упомянутого В НАЗВАНИИ КАРТОЧКИ (единственный источник
для этого поля; обсуждение для таймзоны не используется). Например, «Podgorica-2» ->
'Europe/Podgorica'. Если город не определить однозначно — верни пустую строку.

Название карточки (используй ТОЛЬКО для timezone, язык из него не определяй): {title}

Текст комментариев обсуждения — единственный источник для language (может быть пусто):
{comments}
"""
...
async def detect_card_locale(
    ...
    Источники СТРОГО разведены (живой кейс Черногории, фидбек владельца): language —
    только по тексту `comments`, `title` в это поле не участвует и в нём может быть
    латиница даже у русскоязычной команды («Podgorica-2»); timezone — только по
    `title`, единственный источник для города/зоны."""
