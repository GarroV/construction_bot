...
class ChatRow:
    ...
    # Слоты по местному времени (§7, миграция 0007): timestamp последнего ПРОГОНА тика (не
    # отправки). NULL — прогонов ещё не было. is_digest_due сравнивает его с началом последнего
    # наступившего слота DIGEST_HOURS (9/11/13/15/17/19 по таймзоне чата).
...
async def set_attach_files(pool, chat_id: int, value: bool) -> None:
    """§8: /files on|off и кнопка «📎 Файлы» в /menu — независимый флаг, НЕ трогает
    auto_configured (в отличие от /lang и /time)."""
    await pool.execute("UPDATE chats SET attach_files = $2 WHERE id = $1", chat_id, value)
